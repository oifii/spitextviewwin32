...
TCHAR szTitle[1024]={L"spitextviewwin32title"};					// The title bar text
TCHAR szWindowClass[1024]={L"spitextviewwin32class"};				// the main window class name
                     ...
                     int       nCmdShow)
{
	...
	if(nArgs>13)
	{
		global_statictextcolor_red = atoi(szArgList[13]);
	}
	if(nArgs>14)
	{
		global_statictextcolor_green = atoi(szArgList[14]);
	}
	if(nArgs>15)
	{
		global_statictextcolor_blue = atoi(szArgList[15]);
	}
	if(nArgs>16)
	{
